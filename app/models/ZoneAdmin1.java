...
public class ZoneAdmin1 extends Model {
    ...
    public OpenDataCard getOpenDataCard(String lang) {
        return OpenDataCard.find("SELECT c FROM ZoneAdmin1 z JOIN z.cards c WHERE z.id = ? ORDER BY c.created DESC",
                this.id).first();
    }
    ...
    public static Long getCardIdFromLongLat(float scale, float longitude, float latitude) {
        Long id = null;
        if (scale > 5000000 && scale < 30000000) {
            //@formatter:off
    ...
    public static List<ZoneAdmin1> findAllOpen() {
        return ZoneAdmin1.find("SELECT DISTINCT z FROM ZoneAdmin1 z JOIN z.cards c WHERE c.status>0").fetch();
    }
}
